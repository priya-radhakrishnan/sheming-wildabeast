(function($) {
    ...
    function e(element, dataSeries, graphOptions, tooltips) {
        var defaults = {
            animation: (safari) ? false : true,
            colors: [linkColor, '#888888', '#9FCB00', '#ee5500', '#33bbcc', '#88ddee'],
            dates : {
                format: 'auto'
            },
            errorMessage : false,
            ...
            tooltip: {
                formatter : function(tip, message, allSeries, index, colors) {
                    var toolTipContent = [];
                    if (tip.header) {
                        toolTipContent.push('<div class="rounded flag" style="background-color:#ea4700">' + tip.header + '</div>');
                    };
                    toolTipContent.push('<div class="date">' + tip.dateRange + '</div>');
                    if (tip.demand) {
                        toolTipContent.push('<strong>' + tip.demand + ' ' + tip.unit + '</strong>');
                    } else if (tip.temp) {
                        if (tip.temp || tip.temp === 0) {
                            toolTipContent.push('<div class="weather-tip-container clearfix">');
                        } else {
                            toolTipContent.push('<div class="weather-tip-container no-weather clearfix">');
                        };
                        if (tip.temp || tip.temp === 0) {
                            toolTipContent.push('<div class="temp text-left"><strong style="color:'+colors[0]+'">' + allSeries[0].series[0][index].value + '&deg;</strong>');
                        };
                        toolTipContent.push('<strong class="right" style="color:'+colors[1]+'">' + allSeries[1].series[0][index].value + ' ' + tip.unit + '</strong>');
                        toolTipContent.push('</div>');
                    } else if (tip.solar) {
                        toolTipContent.push('<div class="temp text-left"><strong style="color:'+colors[0]+'">' + allSeries[0].series[0][index].value + ' ' + tip.unit + '</strong>');
                        toolTipContent.push('<strong class="right" style="color:'+colors[1]+'">' + allSeries[1].series[0][index].value + ' ' + tip.unit + '</strong></div>');
                    } else if (tip.allNeighbors || tip.effNeighbors) {
                        if (tip.you) {
                            toolTipContent.push('<div class="text-left">You<strong class="right" style="color:'+colors[0]+'">' + allSeries[0].series[0][index].value + ' ' + tip.unit + '</strong></div>');
                        };
                        if (tip.allNeighbors) {
                            toolTipContent.push('<div class="text-left" style="text-transform:capitalize;">'+neighborLang+'<strong class="right" style="color:'+colors[1]+'">' + allSeries[0].series[1][index].value + ' ' + tip.unit + '</strong></div>');
                        };
                        if (tip.effNeighbors) {
                            toolTipContent.push('<div class="text-left">Efficient<strong class="right" style="color:'+colors[2]+'">' + allSeries[0].series[2][index].value + ' ' + tip.unit + '</strong></div>');
                        };
                    } else if (tip.you) {
                        if (tip.add) {
                            total = 0;
                            if (allSeries[0].series[0]) {total += allSeries[0].series[0][index].value}
                            if (allSeries[0].series[1]) {total += allSeries[0].series[1][index].value}
                            if (allSeries[0].series[2]) {total += allSeries[0].series[2][index].value}
                            if (allSeries[0].series[3]) {total += allSeries[0].series[3][index].value}
                            if (allSeries[0].series[4]) {total += allSeries[0].series[4][index].value}
                            if (allSeries[0].series[5]) {total += allSeries[0].series[5][index].value}
                            if (allSeries[0].series[6]) {total += allSeries[0].series[6][index].value}
                            if (tip.cost){
                                toolTipContent.push('<strong>' + currency + (total).toFixed(2) + '</strong>');
                            } else {
                                toolTipContent.push('<strong>' + (total).toFixed(2) + ' ' + tip.unit + '</strong>');
                            };
                        } else {
                            toolTipContent.push('<strong>' + allSeries[0].series[0][index].value + ' ' + tip.unit + '</strong>');
                        };
                    };
                    if (tip.rebate) {
                        toolTipContent.push('<div class="rebate">' + tip.rebate + '</div>');
                    };
                    if (tip.estimated) {
                        toolTipContent.push('<div class="estimated">' +
                            messages.estimated.text + ' ' + '<a href="#estimated-explaination" class="nofollow" data-trigger-dialog="#estimated-explaination">' +
                            messages.estimated.link + '</a>' + '</div>');
                    };
                    if (tip.drillLink && ami) {
                        toolTipContent.push('<div class="drilldown"><a href="' + tip.drillLink + '">' + tip.drillMessage + '</a></div>');
                    };
...
(function(elroi, $) {
    ...
    function formatDate(format, value, options) {
        var DAY_NAMES_SHORT = ['Sun','Mon','Tue','Wed','Thu','Fri','Sat'],
            ...
            isDoubled,
            i,
            TZmodifier = date.getTimezoneOffset()*60*1000;
        ...
        for (i = 0; i < format.length; i++) {
            ...
            switch (thisChar) {
                case 'd':
                    ...
                    break;
                case 'D':
                    formattedDate += isDoubled ? dayNamesLong[date.getUTCDay()] : dayNamesShort[date.getUTCDay()];
                    break;
                case 'm':
                    if (isDoubled) {
                        ...
                        formattedDate += date.getUTCMonth() + 1;
                    } else {
                        formattedDate += date.getUTCMonth() + 1;
                    }
                    break;
                case 'M':
                    formattedDate += isDoubled ? monthNamesLong[date.getUTCMonth()] : monthNamesShort[date.getUTCMonth()];
                    break;
                case 'y':
                    if (isDoubled) {
                        formattedDate += date.getFullYear();
                    } else {
                        if (date.getUTCFullYear() % 100 < 10) {
                            formattedDate += 0;
                        }
                        formattedDate += date.getUTCFullYear() % 100;
                    }
                    break;
                case 'h':
                    if (isDoubled && date.getUTCHours()  % 12 < 10) {
                        formattedDate += "0";
                    }
                    formattedDate += date.getUTCHours() === 0 ? 12
                        : date.getUTCHours() > 12 ? date.getHours() - 12
                        : date.getUTCHours();
                    break;
                case 'H':
                    formattedDate += isDoubled && date.getUTCHours() < 10 ? "0" + date.getUTCHours() : date.getUTCHours();
                    break;
                case 'n':
                    formattedDate += date.getUTCMinutes() < 10 ? '0' + date.getUTCMinutes() : date.getUTCMinutes();
                    break;
                case 'a':
                    formattedDate += date.getUTCHours() < 12 ? merid[0] : merid[1];
                    break;
                case ':':
        ...
        function drawGrid() {
            ...
            if (graph.options.grid.show) {
                ...
                y = graph.height -
                    graph.padding.bottom +
                    graph.padding.top;
                gridLine = graph.paper.path("M0" + " " + y + "L" + graph.width + " " + y).attr('stroke', graph.options.grid.baseLineColor);
                gridLines.push(gridLine);
            }
        ...
        function drawXLabels(axis) {
            ...
            if (axis.customXLabel) {
                ...
                $(axis.labels).each(function(i) {
                    // fix this dumbshit, this is a hack
                    if (axis.labels[i]=='2&nbsp;pm') {
                        axis.labels[i]='6&nbsp;pm';
                    } else if (axis.labels[i]=='-1&nbsp;pm') {
                        axis.labels[i]='3&nbsp;pm';
                    } else if (axis.labels[i]=='5&nbsp;pm') {
                        axis.labels[i]='9&nbsp;pm';
                    }
            ...
            if (isLineFilled) {
                ...
                if (prevPoint && !isNullPoint) {
                    ...
                    var fillLine = graph.paper.path(fillLineStartPath).attr({
                        'fill': (graph.options.lines.fillColor) ? graph.options.lines.fillColor : color,
                        'stroke-width': 0,
            ...
            rollOverBar.mouseover(function() {
                ...
                if (graph.options.tooltip.show) {
                    ...
                    graph.$tooltip.stop().animate({
                        ...
                        var tipContentHTML = graph.options.tooltip.formatter(graph.tooltips[index], graph.options.messages, graph.allSeries, index, graph.options.colors),
            ...
            $(series).each(function(i) {
                ...
                if (series[i].value || series[i].value === 0 || series[i].pointFlag) {
                    ...
                    if (series[i].pointFlag && (seriesCount === graph.allSeries[0].series.length)) {
                        var $pointFlag = $('<div>Peak day</div>');
                        ...
                        if (graph.options.bars.flagPosition === 'interior' && $pointFlag.outerHeight() < totalBarHeights) {
                            ...
                            pointFlagY = graph.height - y + graph.options.flagOffset + 12;
                        }
                ...
                function() {
                    ...
                    if (graph.options.tooltip.show) {
                        ...
                        graph.$tooltip.stop().animate({bottom: tipY, left:tipX }, 1, function() {
                            var tipContent = graph.options.tooltip.formatter(graph.tooltips[index], graph.options.messages, graph.allSeries, index, graph.options.colors);
